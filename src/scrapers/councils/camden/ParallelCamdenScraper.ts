/**
 * Parallel version of Camden Council Planning Portal Scraper
 */
...
export class ParallelCamdenScraper extends ParallelBaseScraper {
  constructor() {
    const config: ParallelScraperConfig = {
      council: 'Camden',
      baseUrl: 'https://planningrecords.camden.gov.uk',
      rateLimit: 5,
      maxRetries: 3,
      timeout: 30000,
  ...
  protected async getTotalPages(fromDate: Date): Promise<number> {
    try {
      ...
      const paginationText = $('.pagination-info, .page-info').text();
      const totalMatch = paginationText.match(/of (\d+)/i);

      if (totalMatch) {
        const totalResults = parseInt(totalMatch[1], 10);
        return Math.ceil(totalResults / 20); // Assuming 20 results per page
      }

      const hasResults = $('.planning-record, .search-result').length > 0;
      return hasResults ? 10 : 0;
    } catch (error) {
  ...
  async scrapePlanningDetails(reference: string): Promise<PlanningApplication | null> {
    ...
    try {
      const detailUrl = `${this.config.baseUrl}/planning/${encodeURIComponent(reference)}`;
  ...
  private parsePlanningApplication(html: string): PlanningApplication | null {
    try {
      ...
      const address = this.extractText($, '.site-address, #address');
      const proposal = this.extractText($, '.proposal, #description');
      const status = this.extractText($, '.status, #decision');
      const receivedDate = this.extractDate($, '.received-date');
      const validatedDate = this.extractDate($, '.validated-date');
      const decisionDate = this.extractDate($, '.decision-date');
      ...
      const application: Partial<PlanningApplication> = {
        ...
        sourceUrl: `${this.config.baseUrl}/planning/${encodeURIComponent(reference)}`,
      };
  ...
  private parseSearchResults(html: string): PlanningApplication[] {
    ...
    $('.planning-record, .search-result').each((_, element) => {
      try {
        const $elem = $(element);

        const reference = this.extractText($, $elem.find('.reference, a'));
        if (!reference || reference.length < 5) return;

        const address = this.extractText($, $elem.find('.address'));
        const proposal = this.extractText($, $elem.find('.description'));
  ...
  private buildSearchUrl(fromDate: Date, page: number): string {
    ...
    return `${this.config.baseUrl}/search?from_date=${dateStr}&page=${page}`;
  }
