/**
 * Westminster Council Planning Portal Scraper
 */
...
export class WestminsterScraper extends BaseScraper {
  constructor() {
  ...
  private parsePlanningApplication(html: string): PlanningApplication | null {
    try {
      ...
      const reference = this.extractText($, '.caseNumber, #caseNumber');
      if (!reference) return null;

      const address = this.extractText($, '#address');
      ...
      const decisionDate = this.extractDate($, '#decisionDate, #dateDecision');
  ...
  private parseSearchResults(html: string): PlanningApplication[] {
    ...
    $('#searchresults li, .searchresult').each((_, element) => {
      try {
        const $item = $(element);

        const $link = $item.find('a:first, .reference a').first();
        const reference = $link.text().trim();
        if (!reference || reference.length < 5) return;

        const address = this.extractText($, $item.find('.address'));
        const proposal = this.extractText($, $item.find('.description, .proposal'));
