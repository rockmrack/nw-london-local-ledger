/**
 * Harrow Council Planning Portal Scraper
 */
...
export class HarrowScraper extends BaseScraper {
  constructor() {
    const config: ScraperConfig = {
      council: 'Harrow',
      baseUrl: 'https://www.harrow.gov.uk/planning/planning-online',
      rateLimit: 1,
      maxRetries: 3,
      timeout: 30000,
  ...
  async scrapePlanningApplications(fromDate: Date): Promise<PlanningApplication[]> {
    ...
    try {
      ...
      while (page <= maxPages) {
        ...
        const hasNextPage = $('.pagination .next, .next a').length > 0;
  ...
  private parsePlanningApplication(html: string): PlanningApplication | null {
    try {
      ...
      const reference = this.extractText($, '.application-reference, #reference');
      if (!reference) return null;

      const address = this.extractText($, '.site-address, #site-address');
      const proposal = this.extractText($, '.proposal, #proposal, .description');
      const status = this.extractText($, '.status, #status');
      const receivedDateText = this.extractText($, '.date-received, #date-received');
      const decisionDateText = this.extractText($, '.decision-date, #decision-date');
  ...
  private parseSearchResults(html: string): PlanningApplication[] {
    ...
    $('.planning-result, .search-result, .application-item').each((_, element) => {
      try {
        const $item = $(element);

        const reference = this.extractText($, $item.find('.reference, .app-ref'));
        if (!reference || reference.length < 5) return;

        const address = this.extractText($, $item.find('.address, .site-address'));
        const proposal = this.extractText($, $item.find('.proposal, .description'));
  ...
  private buildSearchUrl(fromDate: Date, page: number): string {
    ...
    return `${this.config.baseUrl}/search?dateFrom=${dateStr}&page=${page}`;
  }
