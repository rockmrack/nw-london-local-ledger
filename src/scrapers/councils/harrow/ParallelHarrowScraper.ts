/**
 * Parallel version of Harrow Council Planning Portal Scraper
 */
...
export class ParallelHarrowScraper extends ParallelBaseScraper {
  constructor() {
    const config: ParallelScraperConfig = {
      council: 'Harrow',
      baseUrl: 'https://planningsearch.harrow.gov.uk',
      rateLimit: 5,
      maxRetries: 3,
      timeout: 30000,
  ...
  protected async getTotalPages(fromDate: Date): Promise<number> {
    try {
      ...
      const totalText = $('.results-summary, .total-results').text();
      const totalMatch = totalText.match(/(\d+) results?/i);
      ...
      const hasResults = $('.search-result, .application-item').length > 0;
  ...
  async scrapePlanningDetails(reference: string): Promise<PlanningApplication | null> {
    ...
    try {
      const detailUrl = `${this.config.baseUrl}/planning/details/${encodeURIComponent(reference)}`;
  ...
  private parsePlanningApplication(html: string): PlanningApplication | null {
    try {
      ...
      const reference = this.extractText($, '.ref-number, #app-reference');
      if (!reference) return null;

      const address = this.extractText($, '.location, #location');
      const proposal = this.extractText($, '.development, #proposal');
      const status = this.extractText($, '.decision, #status');
      const receivedDate = this.extractDate($, '.date-received');
      const validatedDate = this.extractDate($, '.date-valid');
      const decisionDate = this.extractDate($, '.date-decided');
      ...
      const application: Partial<PlanningApplication> = {
        ...
        sourceUrl: `${this.config.baseUrl}/planning/details/${encodeURIComponent(reference)}`,
      };
  ...
  private parseSearchResults(html: string): PlanningApplication[] {
    ...
    $('.search-result, .application-item').each((_, element) => {
      try {
        const $elem = $(element);
  ...
  private buildSearchUrl(fromDate: Date, page: number): string {
    ...
    return `${this.config.baseUrl}/search?date_from=${dateStr}&page=${page}`;
  }
